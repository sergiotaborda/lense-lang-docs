...
The main difference from explicit iteration is that explicit iteration normally involves some kind of mutable object , like `List` in this example, while implicit iteration does not.
...
[source, lense ]
----
let cubes : Sequence<Natural> = 1..100.filter(n -> n.isOdd()).map( n -> n ^^ 3));
----

...
public class Hello extends ConsoleApplication {
	public run () {
		let String? argument = Arguments.first();
...
A value can be made mutable by using the `var` annotation instead of `let`. An mutable value is called a <i>variable</i>.
...
----
    ...
    let Function<Number,Number> f = number::sqrt(); // f has a parameter of type Number representing the bounded value of sqrt.
...
public class Polar {
	...
	private let Float angle; // imutable values
	private let Float radius; // imutable values
...
The values angle and radius are imutable, so the class as a whole is imutable. We can inform the compilr of this fact by adding `let` to the class definition.

[source, lense]
----
/**An imutable polar coordinate**/
public let class Polar {
